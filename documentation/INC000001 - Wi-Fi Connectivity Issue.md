...
# INC000001 - Wi-Fi Connectivity Issue

## Current Issue Occurring

User called shortly after starting their shift and reported being unable to access the internet or internal company websites. 
...
* Restarting the workstation did not restore internet or intranet connectivity.

* Verifying the user was connected to the correct corporate Wi-Fi network did not resolve the issue.

* Disconnecting and reconnecting to the corporate Wi-Fi network did not restore connectivity.

* Confirming the user's credentials on a separate workstation showed the account was functioning properly, but the issue remained on the affected workstation.

* Reviewing the network settings did not reveal any visible connection errors.
...
Evidence suggests that the wifi adapter is the cause of the connection issue due to the intermitted connection in device manager and the low wifi signal despite being close to a AP, 
will need to set up a service ticket to have a tech go and verify in person and replace wifi adapter to restore connectivity.
...
* Learned that a 169.254.x.x address can indicate a DHCP issue because Windows will self assign an address when it cannot retrieve one from a DHCP server.
  This provides a useful troubleshooting indicator when investigating network connectivity issues
